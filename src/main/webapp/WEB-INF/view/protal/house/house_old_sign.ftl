...
function submitBtn(){
	...
	tip.confirm("确认发布二手房？", function(){
		load.post("/protal/house/old/add", postData, true, function(data){
	        if (data.status == 0) {
	        	tip.msg(data.msg);
	            window.location.href = "/protal/house/old";
	        } else {
	            tip.msg(data.msg);
	        }
		});
...
<div class="warpe">
    ...
    <div class="part part2">
        <ul>
            ...
            <li>
                <input type="text" id="price" placeholder="房屋预计总价">
                <label class="puff_left">总价：</label>
            </li>
            <li class="clear">
           	    <input type="text" id="build" placeholder="房子建造年份">
                <label class="puff_left">建造年份：</label>
            </li>
            <li class="clear">

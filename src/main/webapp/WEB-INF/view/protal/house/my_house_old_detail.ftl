...
function submitBtn(){
	...
	tip.confirm("确认下架？", function(){
		load.post("/protal/house/old/update", postData, true, function(data){
	        if (data.status == 0) {
	        	tip.msg("取消成功");
	            window.location.href = "/protal/house/old/my";
	        } else {
	            tip.msg("取消失败");
	        }
		});
...
<h1 class="detail-title" id="newId" data-value="${houseOld.id}">${houseOld.title}</h1>

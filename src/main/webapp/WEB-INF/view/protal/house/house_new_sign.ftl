...
$(function(){
	...
	pickout.updated('#decoration');  
	pickout.to({
		el:'#purpose',
		theme: 'clean'
	});
	pickout.updated('#purpose'); 
	$("#pictures").val("");
})
...
function submitBtn(){
	...
	tip.confirm("确认发布新房？", function(){
		load.post("/protal/house/new/add", postData, true, function(data){
	        if (data.status == 0) {
	        	tip.msg(data.msg);
	            window.location.href = "/protal/house/new";
	        } else {
	            tip.msg(data.msg);
	        }
		});
...
<div class="warpe">
    ...
    <div class="part part2">
        <ul>
            ...
            <li>
                <input type="text" id="price" placeholder="房屋预计单价">
                <label class="puff_left">单价：</label>
            </li>
            <li class="clear">
				<select id="decoration" class="pickout" placeholder="请选择装修情况" style="display: none;">
					<option value="" selected>请选择装修情况</option>
					<option value="未装修">未装修</option>
					<option value="普通装修">普通装修</option>
					<option value="精装修">精装修</option>
				</select>
                <label class="puff_left">装修：</label>
            </li>
            <li class="clear">
				<select id="purpose" class="pickout" placeholder="请选择房屋用途" style="display: none;">
					<option value="" selected>请选择房屋用途</option>
					<option value="普通住宅">普通住宅</option>
					<option value="店铺">店铺</option>
				</select>
                <label class="puff_left">用途：</label>
            </li>
        </ul>
    </div>
    <div class="edit_t">

function loadData(){
	$('#dg').datagrid({  
   	    url: '/houseOld/getHouseOldListByPage?title='+$("#txtTile").val()+"&status="+$("#txtStatus").val()
	}); 
}
...
function editOldHouseStatus(status){
    ...
    tip.confirm(msg, function(){
    	...
    	load.post("/houseOld/update", postData, true, function(data){
    		if(data.status == 0){
    			tip.msg(data.msg);
    			loadData();
    		} else {

function loadData(){
	$('#dg').datagrid({  
   	    url: '/houseLease/getHouseLeaseListByPage?title='+$("#txtTile").val()+"&status="+$("#txtStatus").val()
	}); 
}
...
function editLeaseHouseStatus(status){
    ...
    tip.confirm(msg, function(){
    	...
    	load.post("/houseLease/update", postData, true, function(data){
    		if(data.status == 0){
    			tip.msg(data.msg);
    			loadData();
    		} else {
